        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg flex flex-col md:flex-row min-h-[70vh]">
                    ...
                    <nav class="space-y-1">
                        @forelse($chapters as $chap)
                            <a href="{{ route('chapter.show', $chap->slug) }}" 
                               class="block px-3 py-2 rounded-md text-sm font-medium transition-colors duration-150 {{ ($currentChapter && $currentChapter->id === $chap->id) ? 'bg-indigo-600 text-white' : 'text-gray-600 hover:bg-gray-200 hover:text-gray-900' }}">
                                {{ $chap->title }}
                            </a>
                        @empty
                            <p class="text-xs text-gray-500 px-2 italic">Belum ada bab.</p>
                        @endforelse
                    </nav>
                </div>
                ...
                <div class="w-full md:w-3/4 p-6 md:p-8 bg-white">
                    @if($currentChapter)
                        {{-- Ditampilkan jika data chapter ditemukan --}}
                        ...
                        <div class="mt-12 p-6 bg-indigo-50 rounded-xl border border-indigo-100">
                            ...
                            <div id="chat-box" class="space-y-4 max-h-60 overflow-y-auto mb-4 p-3 bg-white rounded-lg border border-gray-100 hidden"></div>
                    ...
                    @else
                        {{-- Ditampilkan jika database kosong --}}
                        <div class="text-center py-16">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 mx-auto text-gray-300 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                            </svg>
                            <h3 class="text-xl font-bold text-gray-700 mb-2">Materi Belum Tersedia</h3>
                            <p class="text-gray-500 text-sm max-w-sm mx-auto">Silakan tambahkan data materi kalkulus / statistika kamu ke tabel database lewat phpMyAdmin terlebih dahulu.</p>
                        </div>
                    @endif
                </div>
